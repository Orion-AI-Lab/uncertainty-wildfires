...
class BBBLayer(nn.Module):
    ...
    def __init__(self, pi, logstd1, logstd2, device, BBB):
    ...
    def resample_with_sharpening(self, grads, eta, std=0.02):
        ...
        for i in range(len(self.sampled_weights)):
            ...
            eps.normal_()
            g = grads[i].detach()
            # Sample from normal wih posterior sharpening
    ...
    def get_kl_sharpening(self, sigma=0.02):
        kl = 0
        for i in range(len(self.sampled_weights)):
            if self.sampled_sharpen_weights:
...
class BBBLinear(BBBLayer):
    ...
    def __init__(self, in_features, out_features, device=None, *args, **kwargs):
        super(BBBLinear, self).__init__(pi=0.25, logstd1=0, logstd2=-6, device=device, BBB=True, *args, **kwargs)
    ...
    def forward(self, inputs):
        ...
        #         biases.append(bias)
        #     weight = torch.stack(weights, dim=0).mean(0)
        #     bias = torch.stack(biases, dim=0).mean(0)
...
class BBBRNN(BBBLayer):
    ...
    def __init__(self, input_size, hidden_size,
                 mode='LSTM', sharpen=False,
                 num_layers=1, batch_first=False,
                 dropout=0, bidirectional=False,
                 device=None,
                 *args, **kwargs):
        super(BBBRNN, self).__init__(pi=0.25, logstd1=0, logstd2=-6, device=device, BBB=True, *args, **kwargs)
        self.mode = mode
        ...
        for layer in range(num_layers):
            for direction in range(num_directions):
                ...
                if self.BBB is True:
                    ...
                    param_names = ['weight_ih_mean_l{}{}', 'weight_ih_logvar_l{}{}', 'weight_hh_mean_l{}{}',
                                   'weight_hh_logvar_l{}{}']
                    param_names += ['bias_ih_mean_l{}{}', 'bias_ih_logvar_l{}{}', 'bias_hh_mean_l{}{}',
                                    'bias_hh_logvar_l{}{}']
                else:
                    layer_params = (
                        w_ih_mean,
    ...
    def forward(self, input, hx=None, grads=None):
        if grads is None:
            self.sample()
            weights = self.sampled_weights
        # elif self.training and self.BBB is True and grads is not None:
        elif self.BBB is True and grads is not None:
            self.sample()
        ...
        #         self.sample()
        #         self.resample_with_sharpening(grads, self.eta)
        #         weights_list.append(self.sampled_sharpen_weights)
        #     weights = torch.tensor(weights_list).mean(0).tolist()
        ...
        # change this line
        # input = input.view(batch_sizes, input.shape[-3], input.shape[-1])
        ...
        return output.squeeze(1), hidden[0].squeeze(0)
